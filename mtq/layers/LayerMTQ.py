...
class LayerMTQ:
    ...
    def fromPDSerie(cls, layer_name, layer_info:pd.Series):
        ...
        return cls(
            id=layer_name,
            name=layer_info[C_ALIAS] if C_ALIAS in layer_info.index else layer_name,
            source=layer_info[C_SOURCE],
            provider=layer_info[C_PROV],
            key_field_name=layer_info[C_ID_NAME] if C_ID_NAME in layer_info.index else "",
            key_field_type=layer_info[C_ID_TYPE] if C_ID_TYPE in layer_info.index else "",
            dt_field_name=layer_info[C_DT] if C_DT in layer_info.index else "",
            dt_field_type=layer_info[C_TYPE_DT] if C_TYPE_DT in layer_info.index else "",
            cs_field_name=layer_info[C_CS] if C_CS in layer_info.index else "",
            cs_field_type=layer_info[C_TYPE_CS] if C_TYPE_CS in layer_info.index else "",
